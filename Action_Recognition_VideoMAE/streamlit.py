...
2. Click Predict button to download the youtube video for inference
""")
buttons = st.container()
output = st.container()
video_container1, video_container2 = output.columns(2)
...
# form
url = form.text_input("Enter a youtube url. (youtube shorts included)", value = 'https://www.youtube.com/watch?v=VofK7BL0Ktg')
predict_button = form.form_submit_button(label = "Predict!")


# if st.session_state["FormSubmitter:form-1. Download"] == True:
#   predict_button = form_col2.form_submit_button(label = "2. Predict")
#   fall_button = form_col3.form_submit_button(label = "3. Fall Detection")

# else:
#   predict_button = form_col2.form_submit_button(label = "2. Predict", disabled = True)
#   fall_button = form_col3.form_submit_button(label = "3. Fall Detection", disabled = True)



if predict_button:
    with st.spinner('Downloading video'):
      if url != 'https://www.youtube.com/watch?v=VofK7BL0Ktg':
        ...
        # display video
        video_container1.write("Action Recognition")
        video_container1.video('test.mp4', format="video/mp4", start_time=0) 

        video_container1.write(f"Video is showing someone <span style='color:green; font-weight: bold'>{result}</span>", unsafe_allow_html=True)

      else:
        
        # Make predictions
        result = predict("https://generalassemblydsi32.s3.ap-southeast-1.amazonaws.com/default.mp4")
        # display video for default url
        video_container1.write("Action Recognition")
        video_container1.video('default.mp4', format="video/mp4", start_time=0) 
        video_container1.write(f"Video is showing someone <span style='color:green; font-weight: bold'>{result}</span>", unsafe_allow_html=True)

        # Fall Detection
        video_container2.write("Fall Detection")
        video_container2.video("https://generalassemblydsi32.s3.ap-southeast-1.amazonaws.com/default_key.mp4", format="video/mp4", start_time=0) 
        video_container2.write("Red bounding box will appear when someone has fallen")
